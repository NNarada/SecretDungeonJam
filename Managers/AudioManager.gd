...
func create_2d_audio_at_location(location, type : SoundEffectSettings.SOUND_EFFECT_TYPE):
	if sound_effect_dict.has(type):
		...
		if sound_effect_setting.has_open_limit():
			...
			new_2D_audio.pitch_scale += Globals.rng.randf_range(-sound_effect_setting.pitch_randomness, sound_effect_setting.pitch_randomness )
...
func create_audio(type : SoundEffectSettings.SOUND_EFFECT_TYPE):
	if sound_effect_dict.has(type):
		...
		if sound_effect_setting.has_open_limit():
			...
			new_audio.pitch_scale += Globals.rng.randf_range(-sound_effect_setting.pitch_randomness, sound_effect_setting.pitch_randomness )
